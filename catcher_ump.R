...
# CHANGE
df = read.csv("Games/20240731-CCBLBourne-1_unverified.csv")
...
plate_x_dim <- c(-9.95/12, 9.95/12, 9.95/12, -9.95/12, -9.95/12) # plus 2 inches on either side
plate_y_dim <- c(1.622, 1.622, 3.37, 3.37, 1.622) #based on trackman dashboard - .5 feet from top of zone
...
final_plot <- function(df) {
  # Filter the dataframe to include only "Ball Called" and "Strike Called"
  #df <- df[df$PitchCall %in% c("Ball Called", "Strike Called"), ]
...
calls <- df_who %>% 
  ...
  select(PitchNo, Pitcher, Batter, BatterTeam, Inning, Balls, Strikes, Outs, PitchCall, PlateLocSide, PlateLocHeight, GameID) 
...
ann_fig <- annotate_figure(fig, top = text_grob(paste0(dt1, "-", opp, "-", name)), 
...
# Open a pdf file
dir.create(paste0("~/Desktop/Wareham/Data/Reports/", dt1, "_", opp), recursive = TRUE)
setwd(paste0("~/Desktop/Wareham/Data/Reports/", dt1, "_", opp))
pdf(paste0(dt1, "_", name, "_", "_Umpire_Report.pdf")) 
# Create a plot
ann_fig
# Close the pdf file
dev.off()
